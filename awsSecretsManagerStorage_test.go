...
func TestNewAwsSecretsManagerStorage(t *testing.T) {
	...
	var testGetSecretValue GetSecretValue = func(keyType string, value []byte, secret *Secret) error {
		return nil
	}
	tests := []struct {
		name string
...
func TestAwsSecretsManagerStorageGetSecret(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			args: args{
				keyID: "k1",
				getSecretValue: func(keyType string, value []byte, secret *Secret) error {
