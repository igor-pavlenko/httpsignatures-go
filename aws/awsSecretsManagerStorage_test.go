...
func TestNewAwsSecretsManagerStorage(t *testing.T) {
	...
	defWant := &SecretsManagerStorage{
		env:                 "",
		storage:             cache2go.Cache("SecretsManagerStorage"),
	...
	var testGetSecretID GetSecretID = func(env string, keyID string, keyType string,
	...
	tests := []struct {
	...
	}{
		{
			...
			want: func() *SecretsManagerStorage {
				res := *defWant
				res.env = "prod"
				return &res
			}(),
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			got.SetGetSecretIDFunc(tt.args.getSecretID)
			got.SetGetSecretValueFunc(tt.args.getSecretValue)
...
func TestDefGetSecretID(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "Not required PublicKey ok",
			args: args{
				env:                "prod",
				keyType:            "PublicKey",
				keyID:              "k1",
				requiredPublicKeys: map[string]bool{"k1": false},
			},
			want: "",
		},
		{
			name: "Required PublicKey ok",
			args: args{
				env:                "prod",
				keyType:            "PublicKey",
				keyID:              "k1",
				requiredPublicKeys: map[string]bool{"k1": true},
			},
			want: "/prod/k1/PublicKey",
		},
		{
			name: "PublicKey ok",
			args: args{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, err := defGetSecretID(tt.args.env, tt.args.keyID, tt.args.keyType, tt.args.requiredPrivateKeys, tt.args.requiredPublicKeys)
...
func (m *mockSecretsManagerClient) GetSecretValue(input *secretsmanager.GetSecretValueInput) (
...
func TestAwsSecretsManagerStorageGetSecret(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			args: args{
				keyID: "k1",
				getSecretID: func(env string, keyID string, keyType string, requiredPrivateKeys map[string]bool, requiredPublicKeys map[string]bool) (string, error) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			sm.SetGetSecretIDFunc(tt.args.getSecretID)
			sm.SetGetSecretValueFunc(tt.args.getSecretValue)
			sm.SetRequiredPrivateKeys(tt.args.requiredPrivateKeys)
			sm.SetRequiredPublicKeys(tt.args.requiredPublicKeys)
			sm.SetAlgorithm(tt.args.algorithm)
